...
function submit_past_activity_onclick() {
  ...
  let dateMil = new Date(reformat_date(data.date)).getTime();
  ...
  fetch(`/store`, {
    ...
    body: JSON.stringify([data, dateMil]), // post body
  })
...
function submit_future_activity_onclick() {
  ...
  //get the correct millisecond after reformating date to mm/dd/yyyy from yyyy-mm-dd
  let dateMil = new Date(reformat_date(data.date)).getTime();
  ...
  fetch(`/store`, {
    ...
    body: JSON.stringify([data, dateMil]), 
...
function translateToUTCDate(date) {
