...
// confused on how to get user info, is it from passport.authenticate?
// what is next() and the .db files
// double check that we set up the user sql correctly

const dbo = require('./databaseOps.js');
const udbo = require('./userDatabaseOps.js')
...
app.get('/name', isAuthenticated, function(request, response) {

  //console.log("before request user is printed")
  console.log("request.user.userData: ", request.user.userData);
  udbo.getUsername(request.user.userData)
  .then((user) => response.send({
    message: "I got your GET request at /name",
    data: user,
  }))
  .catch((error) => {
...
app.post('/store', isAuthenticated, function(request, response, next) {
  response.send({
    message: "I got your POST request at /store"
  });

  let list;
  //console.log("request.body: ", request.body);
  //console.log("request.body[0]: ", request.body[0]);
 
  if (request.body[0].scalar >= 0) {
    // past activity
    list = [request.user.userData, request.body[0].activity, request.body[1], request.body[0].scalar];
  } else {
    // future activity
    list = [request.user.userData, request.body[0].activity, request.body[1], -1];
  }

  dbo.insertItem(list);
});

// STEP 5: reminder GET request
app.get('/reminder', isAuthenticated, function(request, response) {
/*   dbo.run("select * from ActivityTable")
  .then((result) => {
    console.log("run result: ", result);
  }) */
  
  dbo.getAmount(request.user.userData).then((futureActData) => {
    console.log("futureActData printing: ",futureActData);
    response.send({
    message: "I got your GET request at /reminder",
    data: futureActData,
    })
  })
  .catch((error) => {
    console.error('/reminder GET Request Error:', error);
  })

});

// removing old future activities
app.post('/oldFutureRemoval', isAuthenticated, function(request, response) {
  response.send({
    message: "I got your POST request at /oldFutureRemoval"
  });
  dbo.removeOldFuture(request.body, request.user.userData).catch((error) => {
    console.error('/oldFutureRemoval POST Request Error:', error);
  })
})

app.post('/removeCurrentMostRecent', isAuthenticated, function(request, response) {
  response.send({
    message: "I got your POST request at /removeCurrentMostRecent"
  });
  dbo.removeCurrentMostRecent(request.body, request.user.userData).catch((error) => {
    console.error('/removeCurrentMostRecent POST Error:', error);
  })
})

app.get('/week', isAuthenticated, function(request, response) {

  let date = request.url.slice(11, 24);
  let activity = request.url.slice(34);
  console.log(request.url)
  console.log(activity, date)

  dbo.getPastActivity(activity, date, request.user.userData)
  .then((pastLog) => response.send({
    message: "I got your GET request at " + request.url,
    data: pastLog,
  }))
  .catch((error) => {
    console.error('/week?date=xxxx&activity=running POST Error:', error);
  })
}) 
...
function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    udbo.insertUserInfo(userid, username);
    done(null, userid); 
}
